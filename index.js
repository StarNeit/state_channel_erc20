const StateChannelErc20 = artifacts.require('./StateChannelErc20.sol');
const utils = require('./utils.js');

contract('StateChannelErc20', accounts => {
  ...
  describe('constructor', () => {
    ...
    before(async () => {
      ...
      spc = await StateChannelErc20.new(recipient, 60, { from: sender, value: deposit });
    });
  ...
  describe('extendExpiration', () => {
    ...
    before(async () => {
      currentTime = Math.floor(new Date().getTime() / 1000);
      spc = await StateChannelErc20.new(recipient, 1, { from: sender, value: deposit });
    });
